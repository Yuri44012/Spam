  ...
  if (res.status === 403) {
    ...
    if (token) {
      ...
      if (data.errors && data.errors.some(err => err.message === 'Challenge is required to authorize the request')) {
        ...
        await page.goto(`https://www.roblox.com/groups/${groupId}/wall`, { waitUntil: 'domcontentloaded' });
...
async function solveCaptcha(page) {
  const apiKey = 'YOUR_API_KEY_HERE'; // Replace with your actual API key from freecaptchabypass
  try {
